...
// Simular la salida real de certutil que vimos
...
async function testParserReal() {
    console.log('🧪 Probando parser con salida real...');
    
    const detector = new DetectorCertificadosWindows();
    await detector.inicializar();
    
    // Simular la salida real
    const certificados = detector.parsearCertificados(salidaReal);
    
    console.log('📋 Certificados encontrados:', certificados.length);
    console.log(JSON.stringify(certificados, null, 2));
}

testParserReal().catch(console.error);
